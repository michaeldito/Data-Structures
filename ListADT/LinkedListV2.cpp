...
LinkedListV2<ListType>::LinkedListV2(): list_count_(0) {}

template <class ListType>
LinkedListV2<ListType>::LinkedListV2(const LinkedListV2<ListType>& L) {
  ...
  if (original_chain_ptr == nullptr) {
    ...
    // Copy first node.
    head_ = std::make_shared<NodeV2<ListType>>();
    ...
    while (original_chain_ptr != nullptr) {
      ...
      auto new_node_ptr = std::make_shared<NodeV2<ListType>>(next_data);
...
template <class ListType>
LinkedListV2<ListType>::~LinkedListV2() {
  Clear();
}

template <class ListType>
bool LinkedListV2<ListType>::IsEmpty() const {
  return list_count_ == 0 && head_ == nullptr;
}

template <class ListType>
int LinkedListV2<ListType>::GetLength() const {
  return list_count_;
}

template <class ListType>
bool LinkedListV2<ListType>::Insert(int position, const ListType& entry) {
  bool able_to_insert = (position >= 1) && (position <= list_count_ + 1);
  if (able_to_insert) {
    // Create a new node containing the new entry
    auto new_node = std::make_shared<NodeV2<ListType>>(entry);
...
template <class ListType>
bool LinkedListV2<ListType>::Remove(int position) {
...
template <class ListType>
void LinkedListV2<ListType>::Clear() {
  head_ = nullptr;
  list_count_ = 0;
}

template <class ListType>
ListType LinkedListV2<ListType>::GetEntry(int position) const throw(PrecondViolatedExcept) {
...
template <class ListType>
ListType LinkedListV2<ListType>::Replace(int position, const ListType& entry) throw(PrecondViolatedExcept) {
  // Enforce Precondition
...
template <class ListType>
auto LinkedListV2<ListType>::GetNodeAt(int position) const {
